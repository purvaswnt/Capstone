...
df_uber_Aug14 <- readRDS("df_uber_Aug14.rds")
...
Area_counts_Uber_aug <- df_uber_Aug14 %>% group_by(Area) %>% tally()

Area_counts_Uber_aug %>% arrange(desc(n))
Area_counts_Uber_aug <- Area_counts_Uber_aug %>% mutate(Area = tolower(Area))
Area_counts_Uber_aug <- Area_counts_Uber_aug %>% mutate(Area = gsub(" county", "", Area))

gsub(" county", "", Area_counts_Uber_aug$Area)

#Total number of rides requested for all days
nrow(df_uber_Aug14)

# Count of weekdays 
df_uber_Aug14 %>% mutate(day = weekdays(as.Date(Date.Time,format = "%m/%d/%Y"), abbreviate = "F")) %>% 
  group_by(day) %>% tally() %>% arrange(desc(n))

# Count of individual days
df_uber_Aug14 %>% mutate(per_day = as.Date(df_uber_Aug14$DateTime,format = "%m/%d/%Y")) %>% group_by(per_day) %>% tally() %>% arrange(desc(n))

# Count of weekdays 
df_uber_Aug14 %>% mutate(day = weekdays(as.Date(DateTime,format = "%m/%d/%Y"), abbreviate = "F")) %>% 
